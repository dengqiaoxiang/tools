...
;(function(window) {
	...
	u.typeof = function(source) {
		let result = Object.prototype.toString.call(source)
	    return result.replace(/\[object (.*?)\]/,(m,ch)=>ch.toLowerCase())
	}
    ...
    */
    ...
    u.string.toHumpForm = function(str) {
	   if (u.isEmpty(str)) return ''
	 ...
	 * @return {String}
	 */
	u.number.toThousandFilter = function(num) {
		if(!/^-?\d+/g.test(num)) return null
	    return (+num || 0).toString().replace(/^-?\d+/g, m => m.replace(/(?=(\B)(\d{3})+$)/g,',')) 
	}
	
	u.number.toThousandFilter1 = function(num) {
		if(!/^-?\d+/g.test(num)) return null
	...
	u.number.format = function(num, digit){
		const si = [
				{ value: 1E18, symbol: 'E' },
				{ value: 1E15, symbol: 'P' },
				{ value: 1E12, symbol: 'T' },
	...
	u.number.toChinese = function(num) {
	     ...
	     * @param {Object} Num
	     */
	u.number.toMoneyChinese: function(Num) {
	 ...
	 * @return {Array} 排序后的新数组
	 */
	u.array.sortVersion = function(array, order='asc') {
	    return array.sort((a, b) => {
			...
			for (var i =0; i < length; i++){
					let x = aa[i] || 0;
					let y = bb[i] || 0;
					if(x-y !==0 ) return order === 'asc'?x - y:y - x;
			}
		});
	}
	 ...
	 * @param {Number} parId 数组项pid 指定pid
	 */
	u.array.toTree =function (list,id='id', pid='pid', parId = 0){
		let len = list.length
		// 拷贝对象
	...
	u.array.toDelayering = function (list){
		let result = []
		function loop(arr) {
	...
	// 创建一个重复值的数组
	u.array.repeat = function (length,data){
		return Array.apply(null,{length: length}).map(()=>data)
	}
     ...
     * @return {Array}
     */
    u.json.toTreeData = function(data, id = 'id', pid = 'pid', child = 'children') {
        var i
        var ret = []
        var hash = {}
        var len = (data || []).length
	...
	u.date.add = function(date, days) {
		...
		return date.getFullYear() + '-' + tf(month) + '-' + tf(day);
	}
     ...
     */
    u.url.getParams = function(url,name) {
	...
	// 获取url参数, 对象法+分组
	u.url.getQuerys = function(url,name) {
	    var search = (url || window.location).split('?')[1]
	    if (search) {
			var obj = {}
	        decodeURIComponent(search).replace(/([^?&=]+)=([^?&=]*)/g,(res,$1,$2)=>{ 
				obj[$1] = $2
			})
	        return name ? obj[name] : obj
	    }
    ...
    // 验证车牌号正则
    u.validate.carNumber = function(input) {
	...
	// 十六进制颜色正则
	u.validate.color = function(input) {
       return /^([a-fA-F\d]{6}|[a-fA-F\d]{3})$/.test(str);
    }
	
	// 验证密码强度
	u.validate.pwStrength = function(input) {
       let num = 0
	   if([A-Z].test(input)) num++
	   if([a-z].test(input)) num++
	   if([0-9].test(input)) num++
	   if([^\w\s].test(input)) num++
	   return num
    }
	
	// 验证是否是日期
	u.validate.date = function(date) {
	...
	u.html.is = function (str){
		return /<\s*\/?\s*[a-zA-z_]([^>]*?["][^"]*["])*[^>"]*>/g.test(str)
	}
